...
BEGIN
	...
	BEGIN TRY
			...
			PRINT '>> Insertind Data Into : Silver.crm_cust_info'
			...
			PRINT '>> Insertind Data Into : silver.crm_prod_info'
			...
			SELECT 
				prd_id,                                                 -- prd_id
				REPLACE(SUBSTRING(prd_key ,1,5) , '-' , '_') AS cat_id, -- cat_id
				SUBSTRING(prd_key,7, LEN(prd_key)) AS prd_key,          -- prd_key
				prd_nm,                                                 -- prd_nm
				ISNULL(prd_cost,0) AS prd_cost,                         -- prd_cost
				CASE UPPER(TRIM(prd_line))
					 ...
					 WHEN 'T' THEN 'Touring'
					 ELSE 'n/a'
				END AS prd_line,                                        -- prd_line
				CAST(prd_start_dt AS DATE) AS prd_start_dt,             -- prd_start_dt
				CAST(LEAD(prd_start_dt) OVER (
						PARTITION BY prd_key ORDER BY prd_start_dt
					) - 1 AS DATE) AS prd_end_dt,                       -- prd_end_dt
			...
			SELECT 
			   ...
			   CASE WHEN TRIM(cntry) = 'DE' THEN 'Germany'
					WHEN TRIM(cntry) = NULL OR TRIM(cntry) IN ('US','USA') THEN 'United States'
			...
			PRINT '>> Insertind Data Into : silver.erp_px_cat_g1v2'
